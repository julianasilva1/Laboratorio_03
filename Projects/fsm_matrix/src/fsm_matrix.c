...
state_t f_LED1_Apaga_Tempo(state_t curr){ //Estado apaga tempo??
  LEDOn(LED1);
  return LED1_Acende; //Led1 acesso
} // f_Des

state_t f_LED1_Acende_Tempo(state_t curr){
  LEDOff(LED1);
  return LED1_Apaga; //Led1 apagado
} // f_Lig

state_t f_LED1_Apaga_Botao(state_t curr){
  LEDOff(LED2);
  return LED2_Apaga; //Se o botão for pressionado 1 vez led2 apaga
} // f_Des

state_t f_LED1_Acende_Botao(state_t curr){
  LEDOn(LED2);
  return LED2_Acende; //Se o botão for pressionado de novo led2 acende
} // f_Des

state_t f_LED2_Apaga_Tempo(state_t curr){
  LEDOn(LED2);
  return LED2_Acende; //Led2 acesso
} // f_Des

state_t f_LED2_Acende_Tempo(state_t curr){
  LEDOff(LED2);
  return LED2_Apaga; //Led2 apagado
} // f_Lig

state_t f_LED2_Apaga_Botao(state_t curr){
  LEDOff(LED1);
  return LED1_Apaga; //Led1 apaga se o botão for pressionado
} // f_Des

state_t f_LED2_Acende_Botao(state_t curr){
  LEDOn(LED1);
  return LED1_Acende; //Led1 acende
} // f_Des
